...
# Import flagged pre-peak dataset
df_flagged_pre_peak <- read_xlsx(
...
# Create 10-day bins from the minimum DAYS_FROM_MAX_DHW value up to 0
min_val <- floor(min(df_flagged_pre_peak$DAYS_FROM_MAX_DHW, na.rm = TRUE))
breaks <- seq(min_val, 0, by = 10)
if (tail(breaks, 1) < 0) {
  breaks <- c(breaks, 0)
}

DAYS_BIN <- cut(
  df_flagged_pre_peak$DAYS_FROM_MAX_DHW,
  breaks = breaks,
  include.lowest = TRUE,
...
temp_df <- data.frame(
  REGION    = df_flagged_pre_peak$OCEAN_REGION,
  DAYS_BIN  = DAYS_BIN,
...
summary_df <- summary_df %>%
  mutate(
    prop_flagged = n_flagged / n_reports
  )
...
ggsave(
  here("03_flagging", "summaries_and_figures", "figures", "pre_peak", "figure_5.tiff"),
  plot = p,
  device = "tiff",
  width = 180,
  height = 240,
